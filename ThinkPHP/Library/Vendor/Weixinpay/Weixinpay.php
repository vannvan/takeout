    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            $order=array(
                'body'=>$orderinfo['proname'],// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$total_fee,// 订单金额  以(分)为单位（需要根据自己的业务修改）
                'out_trade_no'=>$orderinfo['ordnum'],// 订单号（需要根据自己的业务修改）//坑 这个订单号是测试的固定编号，实际不能重复，微信不允许重复订单号
                'product_id'=>$orderinfo['id'],// 商品id（需要根据自己的业务修改）
                'trade_type'=>'JSAPI',// JSAPI公众号支付
                'openid'=>$openid// 获取到的openid
            );
            //存下订单idnotify要用
            session('orderid',$orderinfo['id']);
            //纪念   2017年11月26日拿下微信支付
            // 统一下单 获取prepay_id
